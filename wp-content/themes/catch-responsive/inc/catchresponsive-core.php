...
/* criado para fisnde estudos */
add_action( 'add_meta_boxes', 'mensagem');
...
function funcaoMensagem(){
	echo 'Isto é apenas umamensagem';
}

add_action( 'add_meta_boxes', 'adicionar_video');
...
function campoInput(){
	global $post;
	$metaBoxInput = get_post_meta($post->ID, 'conteudo_CampoInput', true);
	
	?>
	<input type='text' name='input_video', value='<?php echo $metaBoxInput; ?>'
	<?php
}
...
function update_video(){
	global $post;
	update_post_meta($post->ID, 'conteudo_CampoInput', $_POST['input_video']);
}

function exibe_video(){
	global $post;
	return get_post_meta($post->ID, 'conteudo_CampoInput', true);
}

function listCategoriasLi(){
...
function listCategoriasSelect(){
	wp_dropdown_categories(array(	'name' => 'categorias',
									'hierarchical' => true,
									'show_option_all' => 'Receitas por categoria', 
									'orderby' => 'ID', 
									'style' => 'list', 
									'show_count' => 1, 
									'hide_empty' => 0,
...
function listCategoriasArray(){
	$parametros = array('order' => 'asc',
						'orderby' => 'ID',
						'hide_empty' => 0);
	$categorias = get_categories($parametros);
	/*
	echo '<pre>';	
	print_r($retorno);
	echo '</pre>';
...
add_shortcode('listCatLi', 'listCategoriasLi');	
add_shortcode('listCatSelect', 'listCategoriasSelect');	
add_shortcode('listCatArray', 'listCategoriasArray');	
add_shortcode('listPageArray', 'listPages');	
add_shortcode('listPageChildArray', 'listPagesChild');	
...
$array_categoria = array(
	'label' => 'Categoria Receitas',
	'rewrite' => true,
	'hierarchical' => true
);

register_taxonomy('categorias', 'wp_receitas', $array_categoria);

register_post_type('wp_receitas', $array_argumentos);

/* fim das funções para fins de estudo */
...
if ( ! function_exists( 'catchresponsive_setup' ) ) :
	...
	function catchresponsive_setup() {
		 ...
		 */
		add_theme_support( 'post-formats', array( 'aside', 'image', 'video', 'quote', 'link', 'receitas' ) );
